#!/bin/bash
#
# Build script for ASP.NET wolfSSL FIPS Test Image
#

set -e

# Default values
BASE_IMAGE="cr.root.io/aspnet:8.0.25-bookworm-slim-fips"
IMAGE_NAME="aspnet-fips-test"
IMAGE_TAG="latest"
...
echo ""
echo -e "${BLUE}================================================================${NC}"
echo -e "${BLUE}  Building ASP.NET wolfSSL FIPS Test Image${NC}"
echo -e "${BLUE}================================================================${NC}"
...
if docker build \
    ...
    echo -e "  docker run --rm ${IMAGE_NAME}:${IMAGE_TAG} dotnet-script CryptoTestSuite.cs"
    echo -e "  docker run --rm ${IMAGE_NAME}:${IMAGE_TAG} dotnet-script TlsTestSuite.cs"
    echo ""
    echo -e "${BLUE}To get an interactive shell:${NC}"
    echo -e "  docker run --rm -it ${IMAGE_NAME}:${IMAGE_TAG} /bin/bash"
    echo ""
