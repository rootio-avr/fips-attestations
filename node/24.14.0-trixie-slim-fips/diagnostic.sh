#!/bin/bash
################################################################################
# Node.js 24.14.0 Trixie FIPS - Diagnostic Runner
#
# Purpose: Execute FIPS validation diagnostics for Node.js wolfSSL integration
# Usage:
#   ./diagnostic.sh                                # Run all diagnostics
#   ./diagnostic.sh test-backend-verification.js   # Run specific test
#   ./diagnostic.sh run-all-tests.sh               # Run master test suite
################################################################################
...
IMAGE_NAME="cr.root.io/node:24.14.0-trixie-slim-fips"
DIAGNOSTICS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/diagnostics"

echo ""
echo "================================================================================"
echo -e "${BOLD}${CYAN}Node.js 24.14.0 Trixie FIPS - Diagnostic Runner${NC}"
echo "================================================================================"
echo ""
...
if [ -z "$1" ]; then
    ...
    if [ ! -f "$DIAGNOSTICS_DIR/$TEST_SCRIPT" ]; then
        ...
        ls -1 "$DIAGNOSTICS_DIR"/*.js 2>/dev/null || echo "  (none)"
        echo ""
    ...
    if [[ "$TEST_SCRIPT" == *.js ]]; then
        ...
        echo "Supported types: .js (Node.js), .sh (Shell)"
        exit 1
    fi
