...
Comprehensive test image for validating Node.js 24 with wolfSSL FIPS 140-3 integration.

## Overview

This test image extends the base `node:24.14.0-trixie-slim-fips` image with comprehensive test suites to validate FIPS compliance in a user application context.
...
```bash
docker run --rm node:24.14.0-trixie-slim-fips /test-fips
```
