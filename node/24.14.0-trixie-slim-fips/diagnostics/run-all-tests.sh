#!/bin/bash
#
# Master test runner for Node.js wolfSSL FIPS diagnostics
...
echo ""
echo -e "${BLUE}================================================================${NC}"
echo -e "${BLUE}  Node.js wolfSSL FIPS 140-3 Diagnostic Test Suite${NC}"
echo -e "${BLUE}================================================================${NC}"
echo ""
...
echo -e "${YELLOW}------------------------------------------------------------${NC}"
if node test-backend-verification.js; then
...
echo -e "${YELLOW}------------------------------------------------------------${NC}"
if node test-fips-verification.js; then
...
echo -e "${YELLOW}------------------------------------------------------------${NC}"
if node test-crypto-operations.js; then
...
echo -e "${YELLOW}------------------------------------------------------------${NC}"
if node test-library-compatibility.js; then
...
if [ $PASSED_TESTS -eq $TOTAL_TESTS ]; then
    ...
    echo -e "${GREEN}  Node.js wolfSSL FIPS implementation is ready for production${NC}"
    echo ""
    exit 0
    ...
    echo -e "${RED}  Node.js wolfSSL FIPS implementation has significant issues${NC}"
    echo ""
    exit 2
fi
