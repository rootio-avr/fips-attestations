#!/bin/bash
################################################################################
# Node.js 18 Bookworm FIPS - Diagnostic Runner
#
...
IMAGE_NAME="cr.root.io/node:18.20.8-bookworm-slim-fips"
DIAGNOSTICS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/diagnostics"

echo ""
echo "================================================================================"
echo -e "${BOLD}${CYAN}Node.js 18 Bookworm FIPS - Diagnostic Runner${NC}"
echo "================================================================================"
echo ""
...
if [ -z "$1" ]; then
    ...
    if [[ "$TEST_SCRIPT" == *.js ]]; then
        ...
        echo "Supported types: .js (JavaScript), .sh (Shell)"
        exit 1
    fi
