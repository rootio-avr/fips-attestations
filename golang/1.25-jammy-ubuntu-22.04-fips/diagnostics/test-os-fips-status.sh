...
################################################################################
# Test 5: Application-Level FIPS Environment
################################################################################
echo "[Test 5] Application-Level FIPS Environment Variables"
echo "--------------------------------------------------------------------------------"
...
if [ "$GOLANG_FIPS" = "1" ]; then
    echo -e "${GREEN}✓ SET (value: 1)${NC}"
else
    echo -e "${YELLOW}⚠ NOT SET or wrong value (current: ${GOLANG_FIPS:-unset})${NC}"
    ENV_OK=false
fi
...
if echo "$GODEBUG" | grep -q "fips140=only"; then
    echo -e "${GREEN}✓ CONFIGURED (value: $GODEBUG)${NC}"
else
    echo -e "${YELLOW}⚠ NOT CONFIGURED (current: ${GODEBUG:-unset})${NC}"
    ENV_OK=false
fi
...
if echo "$GOEXPERIMENT" | grep -q "strictfipsruntime"; then
    echo -e "${GREEN}✓ CONFIGURED (value: $GOEXPERIMENT)${NC}"
else
    echo -e "${YELLOW}⚠ NOT CONFIGURED (current: ${GOEXPERIMENT:-unset})${NC}"
    ENV_OK=false
fi
