...
IMAGE_NAME="${IMAGE_NAME:-ubuntu-fips-go:v1.0.0-ubuntu-22.04}"
...
echo "Configuration:"
echo "  - GOLANG_FIPS=1"
echo "  - GODEBUG=fips140=only"
echo "  - GOEXPERIMENT=strictfipsruntime"
echo ""
echo "Expected Behavior:"
echo "  - MD5: BLOCKED (panic from golang-fips/go)"
...
echo "--------------------------------------------------------------------------------"
echo "Configuration:"
echo "  - GOLANG_FIPS=0 (disabled)"
echo "  - GODEBUG= (empty, no FIPS enforcement)"
...
docker run --rm \
  ...
  --entrypoint="" \
  "$IMAGE_NAME" \
  /app/fips-go-demo > "$EVIDENCE_DIR/fips-disabled-output.txt" 2>&1 || true
