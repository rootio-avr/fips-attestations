#!/bin/bash
#
# Build script for Podman wolfSSL FIPS Test Image
#

set -e

# Default values
BASE_IMAGE="cr.root.io/podman:5.8.1-fedora-44-fips"
IMAGE_NAME="podman-fips-test"
IMAGE_TAG="latest"
...
echo ""
echo -e "${BLUE}================================================================${NC}"
echo -e "${BLUE}  Building Podman wolfSSL FIPS Test Image${NC}"
echo -e "${BLUE}================================================================${NC}"
...
if docker build \
    ...
    echo ""
    echo -e "${BLUE}To run with verbose output:${NC}"
    echo -e "  docker run --rm ${IMAGE_NAME}:${IMAGE_TAG} bash podman_fips_test.sh -v"
    echo ""
