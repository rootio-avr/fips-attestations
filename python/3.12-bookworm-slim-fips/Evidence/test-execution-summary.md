# Test Execution Summary - Python 3.12 FIPS
...
This document summarizes all test executions performed against the Python 3.12 wolfSSL FIPS container image
...
**Execution:**
```bash
docker run --rm python:3.12-bookworm-slim-fips python3 /diagnostics/test-backend-verification.py
```
...
**Execution:**
```bash
docker run --rm python:3.12-bookworm-slim-fips python3 /diagnostics/test-connectivity.py
```
...
**Execution:**
```bash
docker run --rm python:3.12-bookworm-slim-fips python3 /diagnostics/test-fips-verification.py
```
...
**Execution:**
```bash
docker run --rm python:3.12-bookworm-slim-fips python3 /diagnostics/test-crypto-operations.py
```
...
**Execution:**
```bash
docker run --rm python:3.12-bookworm-slim-fips python3 /diagnostics/test-library-compatibility.py
```
...
| Metric | Value | Notes |
|--------|-------|-------|
| Image Size | ~380 MB | Includes Python 3.12 + wolfSSL FIPS + wolfProvider (linux/amd64) |
...
| Hostname Verification | Test 4.10 | ✅ PASS - Python 3.12 check_hostname |
...
2. **Single Python Version:** Only Python 3.12 currently supported
...
# Pull or build image
docker pull python:3.12-bookworm-slim-fips
# OR
./build.sh
