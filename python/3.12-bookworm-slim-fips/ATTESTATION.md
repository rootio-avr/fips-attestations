...
Complete compliance and attestation documentation for the Python 3.12 wolfSSL FIPS 140-3 container.
...
- **Python 3.12** (compiled against OpenSSL 3.0.18)
...
```bash
# Build options for Python 3.12 with FIPS support
./Configure \
    --prefix=/usr/local \
    --openssldir=/etc/ssl \
    shared \
...
- Python Source: python.org official source (3.12.x)
...
6. Build Python 3.12 against OpenSSL 3.0.18
...
- Pinned dependency versions (Python 3.12.x, OpenSSL 3.0.18, wolfSSL 5.8.2)
...
# Output: slsa-provenance-python-3.12-bookworm-slim-fips.json
```
...
```json
{
  ...
  "subject": [
    {
      "name": "python:3.12-bookworm-slim-fips",
      "digest": {
        "sha256": "..."
      }
    }
...
trivy image --format spdx-json \
  -o SBOM-python-3.12-bookworm-slim-fips.spdx.json \
...
# Output: vex-python-3.12-bookworm-slim-fips.json
```
  ...
  "statements": [
    {
      ...
      "products": ["python:3.12-bookworm-slim-fips"],
      "status": "not_affected",
...
```bash
# Sign image
cosign sign --key cosign.key python:3.12-bookworm-slim-fips
...
docker run --rm python:3.12-bookworm-slim-fips /test-fips
...
$ docker run --rm python:3.12-bookworm-slim-fips python3 -c \
...
Target: python:3.12-bookworm-slim-fips
...
- Python Source: python.org (Python 3.12.x)
...
**System**: Python 3.12 FIPS Container
**Version**: python:3.12-bookworm-slim-fips
