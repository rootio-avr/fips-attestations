#!/bin/bash
# Build script for Python 3.12 with wolfSSL FIPS 140-3 container

set -e

# Default values
IMAGE_NAME="python"
IMAGE_TAG="3.12-bookworm-slim-fips"
WOLFSSL_PASSWORD=""
PASSWORD_FILE=""
BUILD_ARGS=""
...
usage() {
    ...
    echo "  -t, --tag TAG                Docker image tag (default: 3.12-bookworm-slim-fips)"
...
if [ -z "$WOLFSSL_PASSWORD" ]; then
    ...
    echo -e "${GREEN}Using password from file: $PASSWORD_FILE${NC}"
fi
...
echo ""
echo -e "${BLUE}================================================================${NC}"
echo -e "${BLUE}  Building Python 3.12 with wolfSSL FIPS 140-3 Container${NC}"
echo -e "${BLUE}================================================================${NC}"
...
trap "rm -f $PASSWORD_TMPFILE" EXIT
...
if docker build \
    ...
    if docker run --rm "${IMAGE_NAME}:${IMAGE_TAG}" python3 -c "import ssl; ver = ssl.OPENSSL_VERSION; print('SSL Version:', ver); assert 'OpenSSL' in ver or 'wolfSSL' in ver"; then
        echo -e "${GREEN}✓ SSL version check passed${NC}"
        # Additional check for wolfProvider
        echo -e "${BLUE}  Verifying wolfProvider is loaded...${NC}"
        docker run --rm "${IMAGE_NAME}:${IMAGE_TAG}" openssl list -providers 2>/dev/null | grep -i wolf && echo -e "${GREEN}  ✓ wolfProvider detected${NC}" || echo -e "${YELLOW}  ⚠ wolfProvider status unknown${NC}"
    else
    ...
    if docker run --rm "${IMAGE_NAME}:${IMAGE_TAG}" /test-fips; then
    ...
    if docker run --rm "${IMAGE_NAME}:${IMAGE_TAG}" python3 -c "import ssl; ctx = ssl.create_default_context(); print('✓ SSL context created successfully')"; then
    ...
    if docker run --rm "${IMAGE_NAME}:${IMAGE_TAG}" python3 /opt/wolfssl-fips/bin/fips_init_check.py; then
