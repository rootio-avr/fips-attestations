# Python 3.12 wolfSSL FIPS Developer Guide
...
docker run -it --rm python:3.12-bookworm-slim-fips python3

# Run your application
docker run -it --rm -v $(pwd):/app python:3.12-bookworm-slim-fips python3 /app/your_script.py

# Use as base image
FROM python:3.12-bookworm-slim-fips
...
docker build -t python:3.12-bookworm-slim-fips .
```
...
ldd /usr/lib/python3.12/lib-dynload/_ssl.cpython-312-*.so | grep libssl
```
...
cd python/3.12-bookworm-slim-fips/diagnostics
./run-all-tests.sh
```
    ...
    if fips_required:
        ...
        if not in_fips_container:
            raise RuntimeError(
                ...
                "Run in python:3.12-bookworm-slim-fips container."
            )
...
docker run --rm -v $(pwd):/tests python:3.12-bookworm-slim-fips \
...
# For Docker, use pre-built python:3.12-bookworm-slim-fips image

# On manual install:
apt-get install -y libssl-dev
# Then rebuild Python 3.12
```
...
- [Python SSL Module](https://docs.python.org/3.12/library/ssl.html)
...
**Last Updated:** 2026-03-21
**Tested With:** Python 3.12, wolfSSL 5.8.2 FIPS, OpenSSL 3.0.18
