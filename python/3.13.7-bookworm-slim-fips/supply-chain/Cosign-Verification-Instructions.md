# Cosign Verification Guide for Python 3.13.7 FIPS Image

## Overview

This guide explains how to verify cosign signatures for the Python 3.13.7 FIPS container image (`cr.root.io/python:3.13.7-bookworm-slim-fips`) stored in AWS ECR. The image is signed using Sigstore's keyless signing method with ephemeral keys.
...
**Base:** Python 3.13.7 on Debian Bookworm (Slim)
...
cosign verify \
  ...
  <redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips
```
...
```bash
# Get the digest
docker pull <redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips
docker inspect <redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips --format '{{index .RepoDigests 0}}'
```
...
[{
  "critical": {
    "identity": {
      "docker-reference": "<redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips"
    },
    "image": {
...
1. **Pull from proxy** (for runtime use):
   ```bash
   docker pull cr.root.io/cr.root.io/python:3.13.7-bookworm-slim-fips
   ```

2. **Get the digest** from the pulled image:
   ```bash
   docker inspect cr.root.io/cr.root.io/python:3.13.7-bookworm-slim-fips --format '{{index .RepoDigests 0}}'
   ```
...
```bash
cosign tree <redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips
```

Example output:
```
📦 Supply Chain Security Related artifacts for an image: <redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips
...
cosign download signature <redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips

# View certificate details
cosign verify \
  --certificate-identity-regexp '.*' \
  --certificate-oidc-issuer-regexp '.*' \
  <redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips | jq
```
...
docker run --rm <redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips python3 -c "
import ssl
...
```bash
docker run --rm <redacted_root_ecr_base>/root-reg/cr.root.io/python:3.13.7-bookworm-slim-fips \
