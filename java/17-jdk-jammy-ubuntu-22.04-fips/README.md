...
- **OpenJDK 17 LTS** - Java Development Kit (Eclipse Temurin - Supported until September 2029)
...
docker build \
  ...
  -t java:17-jdk-jammy-ubuntu-22.04-fips \
  .
```
...
docker run --rm java:17-jdk-jammy-ubuntu-22.04-fips
...
docker run --rm -it java:17-jdk-jammy-ubuntu-22.04-fips bash
```
...
```dockerfile
FROM java:17-jdk-jammy-ubuntu-22.04-fips
...
From the `java/17-jdk-jammy-ubuntu-22.04-fips/` directory:

```bash
cd demos-image
./build.sh
# Or with a custom base image:
./build.sh -b cr.root.io/java:17-jdk-jammy-ubuntu-22.04-fips
```

This produces the image `java-17-jdk-jammy-ubuntu-22.04-fips-demos:latest`.
...
# Run the test image
docker run --rm java-17-jdk-jammy-ubuntu-22.04-fips-test-image:latest
```
...
```bash
docker run --rm java:17-jdk-jammy-ubuntu-22.04-fips bash -c \
...
└── compliance/
    ├── SBOM-java-17-jdk-jammy-ubuntu-22.04-fips.spdx.json
    ├── vex-java-17-jdk-jammy-ubuntu-22.04-fips.json
    ├── slsa-provenance-java-17-jdk-jammy-ubuntu-22.04-fips.json
    ├── generate-sbom.sh
    ├── generate-vex.sh
