# Cosign Verification Guide for Java 17 FIPS Image

## Overview

This guide explains how to verify cosign signatures for the Java 17 FIPS container image (`java:17-jdk-jammy-ubuntu-22.04-fips`) stored in AWS ECR. The image is signed using Sigstore's keyless signing method with ephemeral keys.
...
**Base:** Eclipse Temurin OpenJDK 17 on Ubuntu 22.04 (Jammy)
**ECR Repository:** `root-reg/java`
...
```bash
# Get the digest
docker pull <redacted_root_ecr_base>/root-reg/java:17-jdk-jammy-ubuntu-22.04-fips
docker inspect <redacted_root_ecr_base>/root-reg/java:17-jdk-jammy-ubuntu-22.04-fips --format '{{index .RepoDigests 0}}'
```
...
[{
  "critical": {
    "identity": {
      "docker-reference": "<redacted_root_ecr_base>/root-reg/java:17-jdk-jammy-ubuntu-22.04-fips"
    },
    "image": {
...
2. **Get the digest** from the pulled image:
   ```bash
   docker inspect cr.root.io/java:17-jdk-jammy-ubuntu-22.04-fips --format '{{index .RepoDigests 0}}'
   ```
...
3. **Verify against ECR** using the digest:
   ...
   cosign verify \
     --certificate-identity-regexp '.*' \
     --certificate-oidc-issuer-regexp '.*' \
     <redacted_root_ecr_base>/root-reg/java@sha256:<digest-from-step-2>
   ```
...
Example output:
```
📦 Supply Chain Security Related artifacts for an image: <redacted_root_ecr_base>/root-reg/java:17-jdk-jammy-ubuntu-22.04-fips
└── 🔗 https://sigstore.dev/cosign/sign/v1 artifacts via OCI referrer: <redacted_root_ecr_base>/root-reg/java@sha256:<digest>
   └── 🍒 sha256:<signature-digest>
```

### List All Signature Artifacts in ECR

View all signature artifacts for Java images in the ECR repository:
...
cosign download signature <redacted_root_ecr_base>/root-reg/java:17-jdk-jammy-ubuntu-22.04-fips

# View certificate details
cosign verify \
  --certificate-identity-regexp '.*' \
  --certificate-oidc-issuer-regexp '.*' \
  <redacted_root_ecr_base>/root-reg/java:17-jdk-jammy-ubuntu-22.04-fips | jq
```
