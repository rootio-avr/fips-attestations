...
1. **Base Image**: The `java:17-jdk-jammy-ubuntu-22.04-fips` image must be built first
   ```bash
   cd ../../..
   ./build.sh
...
./build.sh -b java:17-jdk-jammy-ubuntu-22.04-fips:custom
...
The build process:
1. Extends base FIPS image (`java:17-jdk-jammy-ubuntu-22.04-fips`)
...
docker run --rm \
  ...
  java-17-jdk-jammy-ubuntu-22.04-fips-test-image:latest
```
...
docker run --rm -it java-17-jdk-jammy-ubuntu-22.04-fips-test-image:latest bash
...
1. **Base Image Not Found**
   ```
   Error: Base image 'java:17-jdk-jammy-ubuntu-22.04-fips' not found!
   ```
