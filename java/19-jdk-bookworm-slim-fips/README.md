...
- **Debian 12 Bookworm** - Container base
...
docker build \
  ...
  -t java:19-jdk-bookworm-slim-fips \
  .
```
...
docker run --rm -it java:19-jdk-bookworm-slim-fips bash
```
...
```bash
docker run --rm java:19-jdk-bookworm-slim-fips bash -c \
...
This image includes complete STIG compliance artifacts demonstrating alignment with DISA Security Technical Implementation Guides for Debian 12 Bookworm (container-adapted):
