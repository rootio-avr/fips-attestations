...
|------|--------|----------|-----------|
| `SBOM-java-19-jdk-bookworm-slim-fips.spdx.json` | JSON | SPDX 2.3 | `generate-sbom.sh` |
| `vex-java-19-jdk-bookworm-slim-fips.json` | JSON | OpenVEX v0.2.0 | `generate-vex.sh` |
| `slsa-provenance-java-19-jdk-bookworm-slim-fips.json` | JSON | SLSA v1.0 | `generate-slsa-attestation.sh` |
...
3. **Continuous Validation**: Run test suite on every deployment:
   ...
   docker run --rm -v $(pwd)/tests:/tests --entrypoint="" \
     java:19-jdk-bookworm-slim-fips bash -c 'cd /tests && ./run-all-tests.sh'
   ```

4. **Audit Log Monitoring**: Mount audit log volume and monitor for policy violations:
   ```bash
   docker run -v /var/log/fips-audit:/var/log java:19-jdk-bookworm-slim-fips
   ```
