...
1. **Base Image**: The `java:19-jdk-bookworm-slim-fips` image must be built first
   ```bash
   cd ../../..
   ./build.sh
...
./build.sh -b java:19-jdk-bookworm-slim-fips:custom
...
The build process:
1. Extends base FIPS image (`java:19-jdk-bookworm-slim-fips`)
...
1. **Base Image Not Found**
   ```
   Error: Base image 'java:19-jdk-bookworm-slim-fips' not found!
   ```
