...
# Explicit FIPS mode
docker run --rm -e FIPS_CHECK=true java:19-jdk-bookworm-slim-fips
...
docker run --rm -e FIPS_CHECK=false java:19-jdk-bookworm-slim-fips

# Quick container for testing
docker run --rm -e FIPS_CHECK=false java:19-jdk-bookworm-slim-fips java -version
...
time docker run --rm -e FIPS_CHECK=true \
  java:19-jdk-bookworm-slim-fips echo "Done"
# ~8 seconds
