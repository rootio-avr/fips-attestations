...
docker build \
  ...
  -t java:21-jdk-jammy-ubuntu-22.04-fips \
  .
```
...
docker run --rm -it java:21-jdk-jammy-ubuntu-22.04-fips bash
```
...
```dockerfile
FROM java:21-jdk-jammy-ubuntu-22.04-fips
...
From the `java/21-jdk-jammy-ubuntu-22.04-fips/` directory:

```bash
cd demos-image
./build.sh
# Or with a custom base image:
./build.sh -b cr.root.io/java:21-jdk-jammy-ubuntu-22.04-fips
```

This produces the image `java-21-jdk-jammy-ubuntu-22.04-fips-demos:latest`.
...
# Run the test image
docker run --rm java-21-jdk-jammy-ubuntu-22.04-fips-test-image:latest
```
...
- OpenJDK 21: GPL v2 with Classpath Exception
...
- [Java Secure Socket Extension (JSSE)](https://docs.oracle.com/en/java/javase/21/security/java-secure-socket-extension-jsse-reference-guide.html)
