...
# Explicit FIPS mode
docker run --rm -e FIPS_CHECK=true java:21-jdk-jammy-ubuntu-22.04-fips
...
OpenJDK Runtime Environment (build 21+36-2238)
OpenJDK 64-Bit Server VM (build 21+36-2238, mixed mode, sharing)
```
...
docker run --rm -e FIPS_CHECK=false java:21-jdk-jammy-ubuntu-22.04-fips

# Quick container for testing
docker run --rm -e FIPS_CHECK=false java:21-jdk-jammy-ubuntu-22.04-fips java -version
...
time docker run --rm -e FIPS_CHECK=true \
  java:21-jdk-jammy-ubuntu-22.04-fips echo "Done"
# ~8 seconds
