...
cd java/11-jdk-jammy-ubuntu-22.04-fips
...
✓ PASS - Java runtime available: openjdk version "11" 2022-09-20
```
...
```bash
# Build demos image
cd java/11-jdk-jammy-ubuntu-22.04-fips/demos-image
./build.sh
...
| **JAVA_HOME** | `test-os-fips-status.sh` | `/usr/local/openjdk-11` |
...
|-----------|------|---------|
| **Sign image** | Cosign | `cosign sign --key cosign.key cr.root.io/java:11-jdk-jammy-ubuntu-22.04-fips` |
| **Verify signature** | Cosign | `cosign verify --key cosign.pub cr.root.io/java:11-jdk-jammy-ubuntu-22.04-fips` |
...
./generate-sbom.sh cr.root.io/java:11-jdk-jammy-ubuntu-22.04-fips
```
...
| **OpenJDK** | 11 | N/A (runtime) | ✅ Configured with FIPS policy |
...
The `java:11-jdk-jammy-ubuntu-22.04-fips` container image **fully satisfies all FIPS POC criteria**:
