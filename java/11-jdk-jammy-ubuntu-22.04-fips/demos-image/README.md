...
```bash
docker run --rm java-11-jdk-jammy-ubuntu-22.04-fips-demos:latest
```
...
./build.sh --base java:11-jdk-jammy-ubuntu-22.04-fips:custom
...
```
Base Image (java:11-jdk-jammy-ubuntu-22.04-fips)
  ...
  └── /opt/wolfssl-fips/bin (FipsInitCheck, etc.)
      ...
      │
      └─> Demos Image (java:11-jdk-jammy-ubuntu-22.04-fips-demos)
...
test-demos:
  stage: test
  script:
    - docker run --rm java:11-jdk-jammy-ubuntu-22.04-fips-demos:$CI_COMMIT_TAG java -cp "/app/demos:/opt/wolfssl-fips/bin:/usr/share/java/*" WolfJceBlockingDemo
    - docker run --rm java:11-jdk-jammy-ubuntu-22.04-fips-demos:$CI_COMMIT_TAG java -cp "/app/demos:/opt/wolfssl-fips/bin:/usr/share/java/*" WolfJsseBlockingDemo
```
...
### Compilation errors
Check that all demo files are compatible with OpenJDK 11 and use proper imports.
