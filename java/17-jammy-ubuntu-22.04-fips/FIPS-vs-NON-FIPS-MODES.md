...
# Explicit FIPS mode
docker run --rm -e FIPS_CHECK=true java:17-jammy-ubuntu-22.04-fips
...
docker run --rm -e FIPS_CHECK=false java:17-jammy-ubuntu-22.04-fips

# Quick container for testing
docker run --rm -e FIPS_CHECK=false java:17-jammy-ubuntu-22.04-fips java -version
...
time docker run --rm -e FIPS_CHECK=true \
  java:17-jammy-ubuntu-22.04-fips echo "Done"
# ~8 seconds
