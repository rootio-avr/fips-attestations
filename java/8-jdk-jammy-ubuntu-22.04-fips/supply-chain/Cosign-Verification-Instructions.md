# Cosign Verification Guide for Java 8 FIPS Image

## Overview

This guide explains how to verify cosign signatures for the Java 8 FIPS container image (`java:8-jdk-jammy-ubuntu-22.04-fips`) stored in AWS ECR. The image is signed using Sigstore's keyless signing method with ephemeral keys.
...
**Base:** Eclipse Temurin OpenJDK 8 on Ubuntu 22.04 (Jammy)
...
```bash
# Get the digest
docker pull <redacted_root_ecr_base>/root-reg/java:8-jdk-jammy-ubuntu-22.04-fips
docker inspect <redacted_root_ecr_base>/root-reg/java:8-jdk-jammy-ubuntu-22.04-fips --format '{{index .RepoDigests 0}}'
```
...
[{
  "critical": {
    "identity": {
      "docker-reference": "<redacted_root_ecr_base>/root-reg/java:8-jdk-jammy-ubuntu-22.04-fips"
    },
    "image": {
...
2. **Get the digest** from the pulled image:
   ```bash
   docker inspect cr.root.io/java:8-jdk-jammy-ubuntu-22.04-fips --format '{{index .RepoDigests 0}}'
   ```
...
Example output:
```
📦 Supply Chain Security Related artifacts for an image: <redacted_root_ecr_base>/root-reg/java:8-jdk-jammy-ubuntu-22.04-fips
...
cosign download signature <redacted_root_ecr_base>/root-reg/java:8-jdk-jammy-ubuntu-22.04-fips

# View certificate details
cosign verify \
  --certificate-identity-regexp '.*' \
  --certificate-oidc-issuer-regexp '.*' \
  <redacted_root_ecr_base>/root-reg/java:8-jdk-jammy-ubuntu-22.04-fips | jq
```
