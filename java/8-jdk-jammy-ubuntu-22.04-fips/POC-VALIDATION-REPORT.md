...
The image is built on **Ubuntu 22.04 Jammy** with **OpenJDK 8** and integrates **wolfSSL FIPS v5.8.2 (Certificate #4718)** through JNI providers (wolfJCE v1.9 and wolfJSSE v1.13), providing cryptographic FIPS enforcement at the Java Cryptography Architecture (JCA/JCE/JSSE) layer without requiring OS-level kernel FIPS mode.
...
cd java/8-jdk-jammy-ubuntu-22.04-fips
...
docker run --rm --entrypoint="" java-8-jdk-jammy-ubuntu-22.04-fips-demos \
...
✓ PASS - Java runtime available: openjdk version "8" 2022-09-20
```
...
```bash
# Build demos image
cd java/8-jdk-jammy-ubuntu-22.04-fips/demos-image
./build.sh
...
docker run --rm cr.root.io/java:8-jdk-jammy-ubuntu-22.04-fips java -version
```
...
| **JAVA_HOME** | `test-os-fips-status.sh` | `/usr/local/openjdk-8` |
...
- `/etc/crypto-policies` not found — RHEL/Fedora-specific; not present on Ubuntu 22.04 Jammy
...
⚠ WARNING - /etc/crypto-policies not found (Ubuntu-based, not RHEL/Fedora)
...
- ✅ Ubuntu-specific: no `/etc/crypto-policies` — FIPS policy enforced via `java.security` instead
...
|------|--------|----------|-----------|
| `SBOM-java-8-jdk-jammy-ubuntu-22.04-fips.spdx.json` | JSON | SPDX 2.3 | `compliance/generate-sbom.sh` (optional; Trivy can also emit CycloneDX) |
| `vex-java-8-jdk-jammy-ubuntu-22.04-fips.json` | JSON | OpenVEX v0.2.0 | `compliance/generate-vex.sh` |
| `slsa-provenance-java-8-jdk-jammy-ubuntu-22.04-fips.json` | JSON | SLSA v1.0 | `compliance/generate-slsa-attestation.sh` |
...
|-----------|------|---------|
| **Sign image** | Cosign | `cosign sign --key cosign.key cr.root.io/java:8-jdk-jammy-ubuntu-22.04-fips` |
| **Verify signature** | Cosign | `cosign verify --key cosign.pub cr.root.io/java:8-jdk-jammy-ubuntu-22.04-fips` |
...
cd java/8-jdk-jammy-ubuntu-22.04-fips/compliance
./generate-sbom.sh

# Or scan a specific registry image
./generate-sbom.sh cr.root.io/java:8-jdk-jammy-ubuntu-22.04-fips
```
...
| **OpenJDK** | 8 | N/A (runtime) | ✅ Configured with FIPS policy |
...
│   Ubuntu 22.04 Jammy Slim base         │   MD5/SHA-1 in TLS/cert/JAR;
...
| Profile | DISA STIG Baseline (Container-Adapted for Ubuntu Jammy) |
| Total Rules | 152 |
| Pass | 128 (84.2%) |
| Fail | 0 (0%) |
...
The `java:8-jdk-jammy-ubuntu-22.04-fips` container image **fully satisfies all FIPS POC criteria**:
