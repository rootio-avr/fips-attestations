...
- **OpenJDK 8 LTS** - Java Development Kit (Eclipse Temurin - Supported until November 2026)
...
docker build \
  ...
  -t java:8-jdk-jammy-ubuntu-22.04-fips \
  .
```
...
docker run --rm java:8-jdk-jammy-ubuntu-22.04-fips
...
docker run --rm -it java:8-jdk-jammy-ubuntu-22.04-fips bash
```
...
docker run --rm \
  -e FIPS_CHECK=false \
  -v /path/to/java.security:$JAVA_HOME/jre/lib/security/java.security \
  java:8-jdk-jammy-ubuntu-22.04-fips
...
```dockerfile
FROM java:8-jdk-jammy-ubuntu-22.04-fips
...
From the `java/8-jdk-jammy-ubuntu-22.04-fips/` directory:

```bash
cd demos-image
./build.sh
# Or with a custom base image:
./build.sh -b cr.root.io/java:8-jdk-jammy-ubuntu-22.04-fips
```

This produces the image `java-8-jdk-jammy-ubuntu-22.04-fips-demos:latest`.
...
# Run the test image
docker run --rm java-8-jdk-jammy-ubuntu-22.04-fips-test-image:latest
```
...
```bash
docker run --rm java:8-jdk-jammy-ubuntu-22.04-fips bash -c \
  "cat \$JAVA_HOME/jre/lib/security/java.security | grep -E 'security.provider.[0-9]'"
```
...
└── compliance/
    ├── SBOM-java-8-jdk-jammy-ubuntu-22.04-fips.spdx.json
    ├── vex-java-8-jdk-jammy-ubuntu-22.04-fips.json
    ├── slsa-provenance-java-8-jdk-jammy-ubuntu-22.04-fips.json
    ├── generate-sbom.sh
    ├── generate-vex.sh
...
- OpenJDK 8: GPL v2 with Classpath Exception
...
- [Java Cryptography Architecture (JCA)](https://docs.oracle.com/en/java/javase/8/security/java-cryptography-architecture-jca-reference-guide.html)
- [Java Secure Socket Extension (JSSE)](https://docs.oracle.com/en/java/javase/8/security/java-secure-socket-extension-jsse-reference-guide.html)
