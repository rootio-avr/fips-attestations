...
This document provides comprehensive instructions for verifying the cryptographic signatures and attestations of the Gotenberg 8.26.0 Debian Trixie FIPS container image using Cosign.
...
curl -o cosign.pub https://cr.root.io/gotenberg/8.26.0-trixie-slim-fips/cosign.pub
...
# Verification for cr.root.io/gotenberg:8.26.0-trixie-slim-fips --
...
```bash
cosign verify --key /path/to/cosign.pub cr.root.io/gotenberg:8.26.0-trixie-slim-fips
```

**Verify and extract claims:**
```bash
# Save verification output to file
cosign verify --key cosign.pub cr.root.io/gotenberg:8.26.0-trixie-slim-fips > verification.json
...
```bash
# Verify multiple tags
for tag in 8.26.0-trixie-slim-fips latest-fips; do
...
cosign triangulate cr.root.io/gotenberg:8.26.0-trixie-slim-fips

# Download signature
cosign download signature cr.root.io/gotenberg:8.26.0-trixie-slim-fips > signature.json
...
verify_gotenberg_image:
  ...
  script:
    ...
    - cosign verify --key cosign.pub cr.root.io/gotenberg:8.26.0-trixie-slim-fips
    - cosign verify-attestation --type slsaprovenance --key cosign.pub cr.root.io/gotenberg:8.26.0-trixie-slim-fips
  only:
    - main
```
...
docker inspect cr.root.io/gotenberg:8.26.0-trixie-slim-fips | jq '.[0].RepoDigests'
...
```bash
# Increase timeout
cosign verify --timeout 60s --key cosign.pub cr.root.io/gotenberg:8.26.0-trixie-slim-fips

# Use local image
docker pull cr.root.io/gotenberg:8.26.0-trixie-slim-fips
cosign verify --key cosign.pub --local-image cr.root.io/gotenberg:8.26.0-trixie-slim-fips
```
