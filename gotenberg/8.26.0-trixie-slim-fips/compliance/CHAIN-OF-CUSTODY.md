...
This document provides a complete chain of custody record for the Gotenberg 8.26.0 Debian Trixie FIPS container image. It tracks the origin, acquisition, build process, validation, and distribution of all components used in the image construction.
...
**Source Acquisition:**
```bash
git clone --depth 1 --branch v8.26.0 \
...
**Integrity Verification:**
- Git tag signed: Check with `git tag -v v8.26.0`
...
| **Tag** | 8.26.0-trixie-slim-fips |
| **Full Image Name** | cr.root.io/gotenberg:8.26.0-trixie-slim-fips |
...
**Push Command:**
```bash
docker tag gotenberg:8.26.0-trixie-slim-fips \
    cr.root.io/gotenberg:8.26.0-trixie-slim-fips

docker push cr.root.io/gotenberg:8.26.0-trixie-slim-fips
```
...
- Tagged: cr.root.io/gotenberg:8.26.0-trixie-slim-fips
- Pushed to registry
...
# Expected: [commit hash for v8.26.0 tag]
```

**2. Verify Build Configuration:**
```bash
# Check build arguments match documented values
docker inspect gotenberg:8.26.0-trixie-slim-fips | jq '.[0].Config.Env'
```

**3. Verify Runtime Behavior:**
```bash
# Run diagnostics
docker run --rm gotenberg:8.26.0-trixie-slim-fips /diagnostics/validate-fips.sh
