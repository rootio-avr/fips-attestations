...
#
# Prerequisites:
#   - cr.root.io/gotenberg:8.26.0-trixie-slim-fips image must be available
...
DEFAULT_TAG="8.26.0-trixie-slim-fips"
TAG="${1:-$DEFAULT_TAG}"
...
if ! docker image inspect cr.root.io/gotenberg:8.26.0-trixie-slim-fips >/dev/null 2>&1; then
    echo -e "${RED}✗ FAILED${NC}: Base image 'cr.root.io/gotenberg:8.26.0-trixie-slim-fips' not found"
    echo ""
...
echo "    docker run -d --name gotenberg-svc --network gotenberg-test-net -p 3000:3000 cr.root.io/gotenberg:8.26.0-trixie-slim-fips"
