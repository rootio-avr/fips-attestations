...
docker run --rm -p 3000:3000 gotenberg:8.26.0-trixie-slim-fips

# Interactive shell
docker run --rm -it --entrypoint bash --user root gotenberg:8.26.0-trixie-slim-fips

# Check version
docker run --rm gotenberg:8.26.0-trixie-slim-fips gotenberg --version
```
...
docker run --rm --entrypoint "" gotenberg:8.26.0-trixie-slim-fips sh -c 'echo $CGO_ENABLED'
# Expected output: 1

# Verify FIPS mode
docker run --rm --entrypoint "" gotenberg:8.26.0-trixie-slim-fips openssl list -providers
...
```bash
docker run --rm -e SKIP_FIPS_CHECK=true gotenberg:8.26.0-trixie-slim-fips
```
...
docker run --rm --entrypoint "" gotenberg:8.26.0-trixie-slim-fips sh -c 'echo test | openssl dgst -md5'
...
docker run --rm --entrypoint "" gotenberg:8.26.0-trixie-slim-fips env | grep CGO
```

**Issue**: "Provider could not be loaded"
```bash
# Check OpenSSL configuration
docker run --rm --entrypoint "" gotenberg:8.26.0-trixie-slim-fips cat /etc/ssl/openssl.cnf

# Check provider library
docker run --rm --entrypoint "" gotenberg:8.26.0-trixie-slim-fips ls -la /usr/local/openssl/lib64/ossl-modules/libwolfprov.so
```
