...
**Project:** Gotenberg 8.30.0 with wolfSSL FIPS 140-3 Container Image
...
```
Image Name: cr.root.io/gotenberg:8.30.0-trixie-slim-fips
Built: 2026-04-16
Size: ~1.2 GB
...
**Method:**
```bash
docker run --rm cr.root.io/gotenberg:8.30.0-trixie-slim-fips \
...
docker run -d --name gotenberg-svc --network gotenberg-demo-net \
  -p 3000:3000 cr.root.io/gotenberg:8.30.0-trixie-slim-fips

docker run --rm --network gotenberg-demo-net \
  gotenberg-demos:8.30.0-trixie-slim-fips /demos/html-to-pdf/run.sh
```
...
**Method:**
```bash
docker run --rm cr.root.io/gotenberg:8.30.0-trixie-slim-fips id
```
...
The Gotenberg 8.30.0 with wolfSSL FIPS 140-3 POC has been thoroughly validated and is **APPROVED FOR PRODUCTION USE**.
