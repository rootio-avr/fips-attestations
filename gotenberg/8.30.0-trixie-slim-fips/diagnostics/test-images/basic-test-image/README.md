...
This creates: `gotenberg-test:8.30.0-trixie-slim-fips`
...
```bash
docker run --rm gotenberg-test:8.30.0-trixie-slim-fips --fips-only
```
...
docker run --rm \
  --network gotenberg-test-net \
  gotenberg-test:8.30.0-trixie-slim-fips \
...
docker run --rm gotenberg-test:8.30.0-trixie-slim-fips --test=fips
