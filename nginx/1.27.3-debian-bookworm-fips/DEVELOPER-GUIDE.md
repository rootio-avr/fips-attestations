...
# Expected output:
# ================================================================================
#   Building Nginx 1.27.3 with wolfSSL FIPS 140-3
# ================================================================================
#
# Phase 1: Downloading sources...
# ✓ nginx-1.27.3.tar.gz downloaded and verified
...
# [Diagnostic output...]
#
# Build complete: cr.root.io/nginx:1.27.3-debian-bookworm-fips
```
      ...
      - name: Security scan
        run: |
          ...
          docker run --rm \
            ...
            aquasec/trivy image cr.root.io/nginx:1.27.3-debian-bookworm-fips

      - name: Push to registry
        if: github.ref == 'refs/heads/main'
        run: |
          echo "${{ secrets.REGISTRY_PASSWORD }}" | docker login -u "${{ secrets.REGISTRY_USER }}" --password-stdin cr.root.io
          docker push cr.root.io/nginx:1.27.3-debian-bookworm-fips
```
...
security-scan:
  stage: scan
  script:
    - trivy image cr.root.io/nginx:1.27.3-debian-bookworm-fips
...
pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                sh 'cd nginx/1.27.3-debian-bookworm-fips && ./build.sh'
            }
        }

        stage('Test') {
            steps {
                sh 'cd nginx/1.27.3-debian-bookworm-fips && ./diagnostic.sh'
                sh 'cd nginx/1.27.3-debian-bookworm-fips/diagnostics/test-images/basic-test-image && ./build.sh'
        ...
        stage('Security Scan') {
            steps {
                sh 'trivy image cr.root.io/nginx:1.27.3-debian-bookworm-fips'
            }
        }
        ...
        stage('Deploy') {
            ...
            steps {
                sh 'docker push cr.root.io/nginx:1.27.3-debian-bookworm-fips'
            }
        }
    }
}
...
|----------|---------|-------------|
| `NGINX_VERSION` | 1.27.3 | Nginx version to build |
...
```bash
# View image layers
docker history cr.root.io/nginx:1.27.3-debian-bookworm-fips

# Inspect image
docker inspect cr.root.io/nginx:1.27.3-debian-bookworm-fips

# Export image
docker save cr.root.io/nginx:1.27.3-debian-bookworm-fips | gzip > nginx-fips.tar.gz
