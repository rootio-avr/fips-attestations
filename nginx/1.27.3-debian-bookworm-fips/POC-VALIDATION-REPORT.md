...
**Project:** Nginx 1.27.3 with wolfSSL FIPS 140-3 Container Image
...
This document presents the validation results for the Nginx 1.27.3 with wolfSSL FIPS 140-3 container image proof of concept (POC). The validation demonstrates successful integration of Nginx with a FIPS 140-3 validated cryptographic module.
...
```
Image Name: cr.root.io/nginx:1.27.3-debian-bookworm-fips
...
**Method:**
```bash
docker run --rm cr.root.io/nginx:1.27.3-debian-bookworm-fips \
...
**Method:**
```bash
docker run -d -p 443:443 --name nginx-test cr.root.io/nginx:1.27.3-debian-bookworm-fips
sleep 3
...
**Method:**
```bash
docker run -d -p 443:443 cr.root.io/nginx:1.27.3-debian-bookworm-fips
...
The Nginx 1.27.3 with wolfSSL FIPS 140-3 POC has been thoroughly validated across all critical dimensions:
