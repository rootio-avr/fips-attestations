# Nginx 1.29.1 with wolfSSL FIPS 140-3

Production-ready Docker image for Nginx 1.29.1 with wolfSSL FIPS 140-3 (Certificate #4718) cryptographic module.

## Overview

This container provides Nginx 1.29.1 on Debian Bookworm with wolfSSL FIPS 140-3 as the cryptographic provider for all SSL/TLS operations. The implementation uses wolfProvider to integrate wolfSSL as the OpenSSL provider system-wide.

### Key Features

✅ **FIPS 140-3 Compliant** - wolfSSL 5.8.2 FIPS (Certificate #4718)
✅ **Nginx 1.29.1** - Latest stable Nginx with wolfSSL integration
...
# Expected output:
#   Nginx 1.29.1 with wolfSSL FIPS 140-3 (Cert #4718)
#   Status: Active
```
...
```bash
docker run -e FIPS_CHECK=false cr.root.io/nginx:1.29.1-debian-bookworm-fips
```
...
1. **Nginx Version 1.29.1 Patch**: wolfssl-nginx repository may not have a specific patch for 1.29.1. The Dockerfile uses direct configuration with `--with-wolfssl` flag. For production, adapt patches from nearby versions (1.27.x or 1.28.1).
...
- [SBOM](compliance/SBOM-nginx-1.29.1-debian-bookworm-fips.spdx.json) (to be created)
- [VEX](compliance/vex-nginx-1.29.1-debian-bookworm-fips.json) (to be created)
- [SLSA Provenance](compliance/slsa-provenance-nginx-1.29.1-debian-bookworm-fips.json) (to be created)
