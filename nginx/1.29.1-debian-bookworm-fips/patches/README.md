# Nginx 1.29.1 FIPS Integration - No Patches Required

## Overview

This directory is reserved for patches, but **no patches are required** for the Nginx 1.29.1 FIPS implementation.
...
### Nginx 1.29.1 Patch Requirement

Since no specific patch exists for 1.29.1, there are three approaches:
...
# Attempt to apply to 1.29.1 (may require manual adjustments)
cd /tmp/nginx-1.29.1
...
If you need to create a custom patch for 1.29.1:
...
diff -Naur nginx-1.29.1-orig nginx-1.29.1 > nginx-1.29.1-wolfssl.patch
```

## Current Build Status

The current Dockerfile **successfully builds Nginx 1.29.1 with wolfSSL** using:
...
If you create or obtain a patch for 1.29.1, place it here:

```
patches/nginx-1.29.1-wolfssl.patch
```
...
COPY patches/nginx-1.29.1-wolfssl.patch /tmp/
RUN cd /tmp/nginx-${NGINX_VERSION} && patch -p1 < /tmp/nginx-1.29.1-wolfssl.patch
```
