...
Complete compliance and attestation documentation for the Nginx 1.29.1 wolfSSL FIPS 140-3 container.
...
- **Nginx 1.29.1** (with SSL module, statically linked with wolfSSL FIPS)
...
```
┌────────────────────────────────────────────┐
│  Nginx 1.29.1 (Master Process, root)       │
...
- Processor: x86_64
- Compiler: GCC
- Integration: Static linking with Nginx 1.29.1
...
- Nginx Source: nginx.org official source (1.29.1)
...
7. Download Nginx 1.29.1 source
...
- Pinned dependency versions (Nginx 1.29.1, OpenSSL 3.0.19, wolfSSL 5.8.2)
...
# Output: slsa-provenance-nginx-1.29.1-fips.json
```
...
```json
{
  ...
  "subject": [
    {
      "name": "nginx:1.29.1-debian-bookworm-fips",
      "digest": {
        "sha256": "..."
      }
    }
  ...
  "statements": [
    {
      ...
      "products": ["nginx:1.29.1-debian-bookworm-fips"],
      "status": "not_affected",
...
```bash
# Sign image
cosign sign --key cosign.key nginx:1.29.1-debian-bookworm-fips
...
docker run --rm nginx:1.29.1-debian-bookworm-fips /test-fips
...
docker run --rm nginx:1.29.1-debian-bookworm-fips fips-startup-check
...
docker run --rm nginx:1.29.1-debian-bookworm-fips openssl version
# Output: OpenSSL 3.0.19

# List providers
docker run --rm nginx:1.29.1-debian-bookworm-fips openssl list -providers
# Output:
# Providers:
#   wolfssl
...
docker run -d -p 443:443 --name nginx-test nginx:1.29.1-debian-bookworm-fips
...
```bash
docker run --rm nginx:1.29.1-debian-bookworm-fips \
...
```bash
docker run -d -p 80:80 -p 443:443 nginx-fips-demos:1.29.1
...
2. **HTTPS-Only Demo**:
```bash
docker run -d -p 8443:443 nginx-fips-demos:1.29.1
# Access: https://localhost:8443/
```

3. **Reverse Proxy Demo**:
```bash
docker run -d -p 443:443 nginx-fips-demos:1.29.1
...
Target: nginx:1.29.1-debian-bookworm-fips
...
- Nginx Source: nginx.org (Nginx 1.29.1)
- Build Date: 2026-03-25T10:00:00Z
- Build Command: ./build.sh
- Build Time: ~10 minutes

## Artifacts
- Image: nginx:1.29.1-debian-bookworm-fips
- Digest: sha256:abcdef123456...
- Size: ~187 MB

## Components
- Nginx: v1.29.1 (statically linked with wolfSSL FIPS)
...
docker run -d --name nginx-test nginx:1.29.1-debian-bookworm-fips
...
**System**: Nginx 1.29.1 wolfSSL FIPS Container
**Version**: nginx:1.29.1-debian-bookworm-fips
