# Cosign Verification Guide for Nginx 1.29.1 FIPS Image

## Overview

This guide explains how to verify cosign signatures for the Nginx 1.29.1 FIPS container image (`cr.root.io/nginx:1.29.1-debian-bookworm-fips`) stored in AWS ECR. The image is signed using Sigstore's keyless signing method with ephemeral keys.
...
**Base:** Nginx 1.29.1 on Debian Bookworm Slim
...
cosign verify \
  ...
  <redacted_root_ecr_base>/root-reg/cr.root.io/nginx:1.29.1-debian-bookworm-fips
```
...
```bash
# Get the digest
docker pull <redacted_root_ecr_base>/root-reg/cr.root.io/nginx:1.29.1-debian-bookworm-fips
docker inspect <redacted_root_ecr_base>/root-reg/cr.root.io/nginx:1.29.1-debian-bookworm-fips --format '{{index .RepoDigests 0}}'
```
...
[{
  "critical": {
    "identity": {
      "docker-reference": "<redacted_root_ecr_base>/root-reg/cr.root.io/nginx:1.29.1-debian-bookworm-fips"
    },
    "image": {
      "docker-manifest-digest": "sha256:27829c6a7a91c6af1b1a5b28d6dc6c3d0c3a2448625b97abaf419e584a29a1b9"
    },
...
```bash
cosign tree <redacted_root_ecr_base>/root-reg/cr.root.io/nginx:1.29.1-debian-bookworm-fips
```

Example output:
```
📦 Supply Chain Security Related artifacts for an image: <redacted_root_ecr_base>/root-reg/cr.root.io/nginx:1.29.1-debian-bookworm-fips
└── 🔗 https://sigstore.dev/cosign/sign/v1 artifacts via OCI referrer: <redacted_root_ecr_base>/root-reg/nginx@sha256:27829c6a7a91c6af1b1a5b28d6dc6c3d0c3a2448625b97abaf419e584a29a1b9
...
cosign download signature <redacted_root_ecr_base>/root-reg/cr.root.io/nginx:1.29.1-debian-bookworm-fips

# View certificate details
cosign verify \
  --certificate-identity-regexp '.*' \
  --certificate-oidc-issuer-regexp '.*' \
  <redacted_root_ecr_base>/root-reg/cr.root.io/nginx:1.29.1-debian-bookworm-fips | jq
```
