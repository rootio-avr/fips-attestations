...
```
Image Name: cr.root.io/redis:7.2.4-alpine-fips
...
**Method:**
```bash
docker run -d --name redis-test cr.root.io/redis:7.2.4-alpine-fips
sleep 2
...
**Method:**
```bash
docker run --rm cr.root.io/redis:7.2.4-alpine-fips sh -c '
...
**Method:**
```bash
trivy image cr.root.io/redis:7.2.4-alpine-fips
```
...
**Method:**
```bash
cat compliance/SBOM-redis-7.2.4-alpine-fips.spdx.json | jq '.packages | length'
```
...
**Method:**
```bash
cat compliance/vex-redis-7.2.4-alpine-fips.json | jq '.vulnerabilities | length'
```
...
├── SBOM-redis-7.2.4-alpine-fips.spdx.json      (Software Bill of Materials)
├── slsa-provenance-redis-7.2.4-alpine-fips.json (SLSA build provenance)
└── vex-redis-7.2.4-alpine-fips.json            (Vulnerability exploitability)
```
