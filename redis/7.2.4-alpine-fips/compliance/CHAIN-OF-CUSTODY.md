...
# Get digest
docker inspect cr.root.io/redis:7.2.4-alpine-fips --format='{{.Id}}'
...
docker run -d --name redis-fips cr.root.io/redis:7.2.4-alpine-fips
...
cat compliance/SBOM-redis-7.2.4-alpine-fips.spdx.json | jq '.packages[].checksums'
```
...
cd fips-attestations/redis/7.2.4-alpine-fips
...
```bash
# View provenance
cat compliance/slsa-provenance-redis-7.2.4-alpine-fips.json | jq .
