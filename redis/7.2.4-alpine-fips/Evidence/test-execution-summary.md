...
**Execution:**
```bash
docker run -t --rm cr.root.io/redis:7.2.4-alpine-fips /diagnostics/test-images/basic-test-image/test-suite.sh
```
...
**Execution:**
```bash
docker run --rm cr.root.io/redis:7.2.4-alpine-fips redis-server --version
```
...
```bash
# Clone repository
git clone <repo-url> && cd fips-attestations/redis/7.2.4-alpine-fips
